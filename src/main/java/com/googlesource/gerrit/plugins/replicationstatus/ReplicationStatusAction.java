...
class ReplicationStatusAction implements RestReadView<ReplicationStatusProjectRemoteResource> {
  ...
  public Response<ProjectReplicationStatus> apply(ReplicationStatusProjectRemoteResource resource)
    ...
    Set<String> remoteUrls = resource.getRemoteUrls();

    logger.atWarning().log("Found remote URLS: " + String.join(",", remoteUrls));
    ...
    try (Repository git = repoManager.openRepository(projectNameKey)) {
      ...
      for (String remote : remoteUrls) {
        ...
        for (Ref r : git.getRefDatabase().getRefs()) {
          ReplicationStatus replicationStatus =
              replicationStatusCache.getIfPresent(
                  ReplicationStatus.Key.create(projectNameKey, remote, r.getName()));

          logger.atWarning().log(
              "**** Looking for into cache: %s|%s|%s", projectNameKey.get(), remote, r.getName());
        ...
        remoteStatuses.put(remote, RemoteReplicationStatus.create(refStatuses));
      }
