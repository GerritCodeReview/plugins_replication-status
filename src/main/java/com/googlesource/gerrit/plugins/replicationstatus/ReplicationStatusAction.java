...
class ReplicationStatusAction implements RestReadView<ReplicationStatusProjectRemoteResource> {
  ...
  private final List<RemoteConfiguration> remoteConfigurations;
  ...
  public Response<ProjectReplicationStatus> apply(ReplicationStatusProjectRemoteResource resource)
    ...
    Optional<RemoteConfiguration> remoteConfig =
        remoteConfigurations.stream()
            .filter(config -> config.getRemoteConfig().getName().equals(remoteName))
            .findFirst();
    ...
    try (Repository git = repoManager.openRepository(projectNameKey)) {
      ...
      ProjectReplicationStatus.ProjectReplicationStatusResult overallStatus =
          getOveralStatus(remoteStatuses);
  ...
  private ProjectReplicationStatusResult getOveralStatus(
  ...
  private Optional<String> getTargetURL(
      Project.NameKey projectNameKey, RemoteConfiguration config, String url) {
    try {
  ...
  private URIish getURI(RemoteConfiguration config, URIish template, Project.NameKey project) {
