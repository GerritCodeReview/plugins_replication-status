...
import static com.gerritforge.gerrit.plugins.replicationstatus.ProjectReplicationStatus.ProjectReplicationStatusResult;
import static com.gerritforge.gerrit.plugins.replicationstatus.ProjectReplicationStatus.ProjectReplicationStatusResult.FAILED;
import static com.gerritforge.gerrit.plugins.replicationstatus.ProjectReplicationStatus.ProjectReplicationStatusResult.OK;
import static com.gerritforge.gerrit.plugins.replicationstatus.ProjectReplicationStatus.create;
import static com.gerritforge.gerrit.plugins.replicationstatus.RefReplicationStatus.CACHE_NAME;
...
class ReplicationStatusAction implements RestReadView<ProjectResource> {
  ...
  public Response<ProjectReplicationStatus> apply(ProjectResource resource)
    ...
    checkIsOwner(projectNameKey);

    ProjectReplicationStatusResult overallStatus = OK;
    ...
    try (Repository git = repoManager.openRepository(projectNameKey)) {
      ...
      ProjectReplicationStatus projectStatus =
          create(refStatuses, overallStatus, resource.getName());
  ...
  private void checkIsOwner(Project.NameKey project) throws PermissionBackendException {
    if (!permissionBackend.currentUser().testOrFalse(GlobalPermission.ADMINISTRATE_SERVER)
      ...
      throw new PermissionBackendException("Administrate Server or Project owner required");
    }
  }
}
