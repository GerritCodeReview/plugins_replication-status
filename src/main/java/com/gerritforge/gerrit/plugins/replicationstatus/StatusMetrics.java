...
public class StatusMetrics implements LifecycleListener {
  ...
  private void addGlobalLatencyMetric() {
    ...
    Runnable metricCallBack =
            () -> {
              latencyMetric.set(statusStore.getGlobalLastReplicationTime());
            };
    RegistrationHandle registrationHandle = metricMaker.newTrigger(latencyMetric, metricCallBack);
  ...
  private void addLatencyMetricFor(String projectName) {
    ...
    Runnable metricCallBack =
            () -> {
              latencyMetric.set(statusStore.getLastReplicationTime(projectName).orElse(0L)); //XXX Map here!
            };
    RegistrationHandle rh = metricMaker.newTrigger(latencyMetric, metricCallBack);
    log.info("Added callback for " + projectName);

    projectsWithMetrics.add(projectName);
    registeredMetrics.add(rh);
    triggers.add(metricCallBack);
  }
}
