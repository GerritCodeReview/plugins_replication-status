...
import static com.gerritforge.gerrit.plugins.replicationstatus.RemoteReplicationStatus.ReplicationStatusResult;
import static com.gerritforge.gerrit.plugins.replicationstatus.RemoteReplicationStatus.create;
  ...
  private static String emptyReplicationStatus(Project.NameKey project) {
    return gson.toJson(
        ProjectReplicationStatus.create(
            Collections.emptyMap(), ProjectReplicationStatusResult.OK, project.get()));
  }

  private static String successReplicationStatus(Project.NameKey project, long when) {
    return projectReplicationStatus(
        project, when, ProjectReplicationStatusResult.OK, ReplicationStatusResult.SUCCEEDED);
  }

  private static String failedReplicationStatus(Project.NameKey project, long when) {
    return projectReplicationStatus(
        project, when, ProjectReplicationStatusResult.FAILED, ReplicationStatusResult.FAILED);
  }

  private static String projectReplicationStatus(
      Project.NameKey project,
      long when,
      ProjectReplicationStatusResult projectReplicationStatusResult,
      ReplicationStatusResult replicationStatusResult) {
    return gson.toJson(
        ProjectReplicationStatus.create(
            ImmutableMap.of(
                REF_MASTER,
                RefReplicationStatus.create(
                    ImmutableMap.of(REMOTE, create(replicationStatusResult, when)))),
